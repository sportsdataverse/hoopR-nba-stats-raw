...
Why this exists
---------------
`season_capture.write_payload` had no guard, and resume is `path.exists()` --
presence, not content. So a failed fetch that returned `{}` was written to disk
and every later sweep counted it "present", never refetched it, and reported the
season complete. A backfill no-ops. hoopR-nba-stats-raw reached 3,347 such files
and wehoop-wnba-stats-raw 3,872.

The guard now refuses to persist a contentless payload, but it cannot undo the
files already on disk: they still exist, so they are still skipped. This script
deletes them and refetches exactly those (endpoint, season, variant) tuples.

A live probe confirmed the data is really there -- all seven
leaguedashteamshotlocations measure types return 30 rows, leaguedashptteamdefend
30, matchupsrollup 2,283 -- so these were failed fetches, not "no data".

Safety
------
* Only files whose size is <= 2 bytes are touched. That is exactly `{}` / `[]`.
  It is NOT "small": playbyplayv3/1996/0029500001.json is a valid 201-byte
  envelope whose `game.actions` is legitimately empty, and must survive.
* The deleted files are tracked in git, so `git checkout -- nba_stats/` restores
  them if a run goes wrong.
...
def store_root() -> Path:
    ...
    return Path(os.environ.get(STORE_ENV) or str(REPO.joinpath(*STORE_SUBDIR)))


def find_empty(root: Path) -> dict[int, set[tuple[str, str | None]]]:
    """(season) -> {(endpoint, variant)} for every contentless payload on disk."""
    out: dict[int, set[tuple[str, str | None]]] = defaultdict(set)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.seasons:
        ...
        empty = {s: v for s, v in empty.items() if lo <= s <= hi}
    if args.endpoint:
    ...
    import sportsdataverse.nba.nba_stats as stats
    ...
    if pool:
        ...
        # Same contract as the main sweep: un-proxied stats.nba.com calls HANG
