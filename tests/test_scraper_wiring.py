"""Contract tests for the objects `_capture_runtime.main()` wires together.
...
def test_every_proxyhealth_attribute_the_scraper_uses_exists() -> None:
    ...
    assert not missing, f"_capture_runtime calls health.{missing} which ProxyHealth lacks"
...
def test_every_roundrobin_attribute_the_scraper_uses_exists() -> None:
    ...
    assert not missing, f"_capture_runtime calls rr.{missing} which RoundRobin lacks"
